...
# Hyperparameters:
# Neural Network
nodes_per_layer = 512                 # nodes in each fully connected layer
...
# Plots
stats_on_data = True
data_probing_plots = True
lookup_table_plots = True
evaluate_predictors = True
...
# @author: Joao (Modified by Assistant)
# """
...
# # Import the updated train_test_func module
# import train_test_func as func
# import matplotlib.pyplot as plt
...
# # Variables to loop
# ai_strategies = ['NN', 'XGBoost']       # 'KNN', 'LT', 'NN', 'XGBoost'
# norm_types = [1]                        # [1,2,3,4,5]
# scen_idxs = [1]  # np.arange(1,9+1)     # [1,2,3,4,5,6,7,9]
# n_beams_list = [64]                     # [8, 16,32,64]
# noises = [0]                            # position noise in meters
...
# # Variables constant across simulation
# use_cal_pos_for_scen = [3, 4, 8, 9]     # These scenarios needed calibration.
# max_samples = int(1e5)                  # max samples to consider per scenario
# n_avgs = 5                              # number of runs to average
# train_val_test_split = [60, 20, 20]     # 0 on val uses test set to validate.
# top_beams = np.arange(5) + 1            # Collect stats for Top X predictions
# force_seed = -1                         # When >= 0, sets data randomization seed.

# # Hyperparameters:
# # Neural Network
# nodes_per_layer = 1024                  # Increased nodes for deeper network
# layers = 5 + 2                          # Increased number of layers
...
# }
# use_best_xgb_params = False             # If True, use predefined best params

# # KNN (if used)
# n_knn = 5                               # number of neighbors
# use_best_n_knn = False                  # if True, ignores the value above.
# BEST_N_PER_SCENARIO_KNN = [5, 24, 65, 28, 9, 5, 13, 80, 54]

# # Lookup Table (if used)
# n_lookuptable = 25                      # number of divisions of each coordinate
# use_best_n_lookuptable = False          # if True, ignores the value above.
...
#     # Assign beam values (and automatically downsample if n_beams != 64)
#     if n_beams not in [8, 16, 32, 64]:
#         raise Exception('Invalid number of beams')
...
#         if ai_strategy == 'XGBoost':
#             n = -1  # placeholder
#             runs_folder_name = 'XGBoost'
...
#             if train_val_test_split[1] == 0 or ai_strategy in ['KNN', 'LT', 'XGBoost']:
#                 train_samples = train_val_samples
#                 val_samples = test_samples
#             else:
#                 train_ratio = np.sum(train_val_test_split[:2]) / 100
...
#         # 1- XGBoost
#         if ai_strategy == 'XGBoost' and run_idx <= n_avgs:
#             print("Training XGBoost model...")
#             if use_best_xgb_params:
#                 xgb_best_params = {
#                     'n_estimators': 200,
#                     'max_depth': 8,
#                     'learning_rate': 0.05,
...
#                 xgb = XGBClassifier(objective='multi:softprob', num_class=n_beams)
...
#             # Save the model
#             func.save_xgboost_model(model, run_folder)

#             # Predict
#             pred_probs = model.predict_proba(x_test)
#             pred_beams = np.argsort(-pred_probs, axis=1)

#         # 2- NN (with advanced architecture)
#         if ai_strategy == 'NN' and run_idx <= n_avgs:
#             if quantize_input:
#                 n_bins = 200
#                 x_train = np.round(x_train / (1 / n_bins)) * (1 / n_bins)
...
#             model = func.Advanced_NN(x_train.shape[-1], n_beams, nodes_per_layer, layers)
...
#             # Average the number of correct guesses (over the total samples)
#             acc = np.round(total_hits / n_test_samples, 4) * 100

#             print(f'{ai_strategy} Results:')
#             for i in range(n_top_stats):
#                 print(f'\tAverage Top-{top_beams[i]} accuracy {acc[i]:.2f}%')

#             # Save Test acc to file
#             np.savetxt(os.path.join(run_folder, 'test_accs.txt'),
#                        acc, fmt='%.2f')

#             # Compute power loss ratio
#             power_loss_ratio = np.zeros(n_test_samples)
#             for i in range(n_test_samples):
#                 noise = np.min(y_test_pwr[i, :]) / 2
...
#             print(f"{ai_strategy} Mean Power Loss (dB): {mean_power_loss_db:.4f}")

#             np.savetxt(os.path.join(run_folder, 'power_loss.txt'),
#                        np.stack((mean_power_loss_db, 0)))  # needs to be 1D..

#         # ---------- Phase 7: Compute average across runs (if applicable) ---------
#         if run_idx >= n_avgs:
#             func.compute_and_save_average_results(ai_strategy, runs_folder, n_avgs, top_beams)
